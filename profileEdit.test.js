...
describe('User Profile Edit Functionality', () => {
...
test('Valid user data should resolve successfully', async () => {
  ...
  // Mock the profileEditCred function to resolve successfully for valid user data
  tool.profileEditCred = jest.fn().mockResolvedValue({ success: true });

  // Start the server and store the server instance in the server variable
  server = app.listen(3000);

  // Call the function with valid user data
  const result = await tool.profileEditCred(user);
...
test('Invalid user data should reject with an error message', async () => {
  ...
  // Mock the profileEditCred function to reject with an error message for invalid user data
  tool.profileEditCred = jest.fn().mockRejectedValue(new Error('All fields are required'));
  ...
  try {
    await tool.profileEditCred(user);
  } catch (error) {
